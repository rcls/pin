...
class WeierstraussCurve:
    ...
    @staticmethod
    def curve(x, y, a, p) -> 'WeierstraussCurve':
        if a >= p or a <= p // -2:
            a = a % p
        assert a != 0
        b = (y * y - x * x % p * x - a * x) % p
        assert b != 0
        return WeierstraussCurve(a, b, p, -p)
...
def test_basic() -> None:
    curve = WeierstraussCurve.curve(4, 5, 7, 65537)
    z = (4, 5)
    curve.verify(z)
    print(curve)
    z2 = curve.add(z, z)
...
def qtry_one(n: int, B1: int) -> None:
    ...
    curve = WeierstraussCurve.curve(x, y, a, n)
    Q = (x,y)
